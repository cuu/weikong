...
int main( void )
{
  ...
  size = osal_nv_item_len( ZCD_NV_CHANLIST);
  
  if( size  <= sizeof(zgDefaultChannelList) && size != 0  )
  {
  ...
  size = osal_nv_item_len( ZCD_NV_PANID);
  /// 改变 panid ，在启动时
  if( size  <= sizeof(zgConfigPANID) && size != 0  )
  {
  ...
  {
      zgConfigPANID = ZDAPP_CONFIG_PAN_ID;
  }

  size = osal_nv_item_len( 0x0201);
  
  if( size <= sizeof(start_flag) && size != 0 )
  {
    osal_nv_item_init( 0x0201, sizeof(start_flag), &start_flag);
    osal_nv_read(0x0201,0,  sizeof(start_flag), &start_flag);
  }else
  {
    start_flag = 0;
  }
